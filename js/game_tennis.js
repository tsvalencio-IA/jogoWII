...
// STATUS: RALLY MULTIPLIER, DYNAMIC CAMERA ZOOM + INJEÇÃO 20/10 (SPIN EFFECT & CHEER)
// =============================================================================
...
(function() {
    ...
    const MathCore = {
        ...
        predictX: (b, targetZ) => {
            if (Math.abs(b.vz) < 0.1) return b.x;
            let time = Math.abs((targetZ - b.z) / b.vz);
            let finalX = b.x + (b.vx * time);
            return Number.isFinite(finalX) ? finalX : 0;
        },
        predictY: (b, targetZ) => {
            if (Math.abs(b.vz) < 0.1) return b.y;
            let time = Math.abs((targetZ - b.z) / b.vz);
            let finalY = b.y + (b.vy * time) + (0.5 * CONF.GRAVITY * time * time);
            return Number.isFinite(finalY) ? finalY : -200;
        }
    };
    ...
    const Game = {
        state: 'MODE_SELECT', 
        timer: 0, idleTimer: 0, endTimer: 0, serveTimeout: 0,
        ...
        cleanup: function() {
            ...
            if(window.System && window.System.canvas) {
                window.System.canvas.onclick = null; window.System.canvas.onmousemove = null;
                window.System.canvas.ontouchstart = null; window.System.canvas.ontouchmove = null;
            }
        },
        ...
        sfx: function(action) {
            try {
                if (window.Sfx) {
                    if (action === 'coin') window.Sfx.play(1200, 'sine', 0.1);
        ...
        setupInput: function() {
            ...
            window.System.canvas.onclick = (e) => {
                ...
                if (this.state === 'MODE_SELECT') {
                    ...
                    this.executeAimAssistHit('p1', 0, 0, 200);
                }
            ...
            try { 
                ...
                if (this.state === 'MODE_SELECT') { 
                    ...
                    if (this.state === 'IDLE') {
                        ...
                        if (this.idleTimer > 1000) { this.state = 'SERVE'; this.idleTimer = 0; this.serveTimeout = 0; }
                    ...
                    if (dt > 0) {
                        ...
                        if (this.state === 'RALLY' || this.state === 'SERVE') {
                            if (!this.isOnline || this.isHost) { this.updatePhysics(dt); if (!this.isOnline) this.updateAI(); this.updateRules(dt); } 
        ...
        processPose: function(pose, w, h) {
            ...
            if (this.useMouse) {
                ...
                const targetY = MathCore.lerp(-200, 300, ny); 
        ...
        executeAimAssistHit: function(who, offX=0, offY=0, handSpeed=0) {
            ...
            if (velY < -80 && isP1 && !isBlock && !this.useMouse) {
                ...
                this.ball.vz = dz / timeToReach;
                this.ball.vy = (dy - (0.5 * CONF.GRAVITY * timeToReach * timeToReach)) / timeToReach;
                if(isP1) this.addMsg("BALÃO!", "#f1c40f");
            }
            ...
            // --- INJEÇÃO 20/10: CROWD CHEER ---
            if (this.rallyCount === 10) {
                this.addMsg("TORCIDA À LOUCURA!", "#f1c40f");
                this.playHitSound(800, 20); 
        ...
        updatePhysics: function(dt) {
            ...
            for(let s=0; s<steps; s++) {
                ...
                // --- INJEÇÃO 20/10: MAGNUS EFFECT (SPIN PHYSICS) ---
                b.vx += (b.spinY || 0) * 0.003; 
                b.vy += (b.spinX || 0) * 0.003;
                ...
                if (b.y >= CONF.TABLE_Y && previousY < CONF.TABLE_Y) { 
                    if (Math.abs(b.x) <= CONF.TABLE_W/2 && Math.abs(b.z) <= CONF.TABLE_L/2) {
                        ...
                        if (this.lastHitter === side) { 
                            this.scorePoint(side === 'p1' ? 'p2' : 'p1', "DOIS TOQUES"); return; 
                        } else {
                            this.ball.bounceCount++;
                            if(this.ball.bounceCount >= 2) { 
                                this.scorePoint(side === 'p1' ? 'p2' : 'p1', "PONTO!"); return; 
                            }
                        }
                    }
                }
            ...
            if (Math.abs(b.z) < 20 && b.y > -CONF.NET_H && b.y < CONF.TABLE_Y) {
                b.vz *= -0.2; b.vx *= 0.5; this.shake = 5; this.playHitSound(100); b.lastHitBy = null;
            }
            ...
            if (this.ball.vz < 0 && this.ball.lastHitBy !== 'p1') {
                if (this.ball.z < this.p1.z + 400 && this.ball.z > this.p1.z - 200) {
                    ...
                    if (Math.sqrt(dx*dx + dy*dy) < CONF.PADDLE_HITBOX) { 
                        this.executeAimAssistHit('p1', dx, dy, speed + 50);
                    }
                }
            }
        },
        ...
        checkPaddleHit: function() {
            if (this.ball.vz < 0 && this.ball.lastHitBy !== 'p1') {
                if (this.ball.z < this.p1.z + 400 && this.ball.z > this.p1.z - 200) {
                    ...
                    let handSpeed = Math.sqrt(this.p1.vx**2 + this.p1.vy**2);
            ...
            if (this.state === 'SERVE') {
                ...
                if (this.serveTimeout > 4000) {
                    if (this.server === 'p1') {
                        ...
                        this.executeAimAssistHit('p1', 0, 0, 100);
                    } else { 
                        if (!this.isOnline || this.isHost) this.executeAimAssistHit('p2', 0, 0, 80); 
                    }
                    this.serveTimeout = 0;
                }
            }
        ...
        calculateAITarget: function() {
            ...
            let predX = MathCore.predictX(this.ball, this.p2.z); 
            let predY = MathCore.predictY(this.ball, this.p2.z);
            ...
            const humanErrorX = baseError * this.activeAIProfile.difficultyFactor * speedFactor;
            const humanErrorY = 50 * this.activeAIProfile.difficultyFactor * speedFactor;
            
            this.p2.targetX = predX + (Math.random() - 0.5) * humanErrorX;
            this.p2.targetY = predY + (Math.random() - 0.5) * humanErrorY; 
        },
        ...
        updateAI: function() {
            ...
            if (dist > 0) {
                let moveSpeed = Math.min(dist * this.activeAIProfile.speed, maxSpeedX);
                ai.vx = (dx / dist) * moveSpeed;
                ai.vy = Math.min((dy / dist) * moveSpeed, maxSpeedY);
        ...
        renderLobby: function(ctx, w, h) {
            ...
            if (this.isHost) {
                ...
                if (pCount >= 2) {
                    ...
                    ctx.fillStyle = "white"; ctx.font = "bold 24px 'Russo One'"; ctx.fillText("INICIAR PARTIDA", w/2, h*0.7 + 45);
                } else {
                    ctx.fillStyle = "#e67e22"; ctx.font = "bold 20px 'Russo One'"; ctx.fillText("AGUARDANDO OPONENTE...", w/2, h*0.7 + 40);
                }
            } else {
        ...
        drawBall: function(ctx, w, h) {
            ...
            if (this.ball.y < CONF.FLOOR_Y) {
                ...
                if (shadowPos.visible && Number.isFinite(shadowPos.x)) {
                    ...
                    this.safeEllipse(ctx, shadowPos.x, shadowPos.y, bSr*1.5, bSr*0.5, `rgba(0,